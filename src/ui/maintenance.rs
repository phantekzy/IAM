...
pub fn page_maintenance(ui: &mut egui::Ui, app: &mut App) {
    ...
    panneau().show(ui, |ui| {
        ...
        egui::Grid::new("rep_add")
            ...
            .show(ui, |ui| {
                ...
                egui::ComboBox::from_id_source("rep_v_cb")
                    .selected_text(
                        app.voitures
                            .get(app.rep_voiture_idx)
                            .map(|v| v.modele.as_str())
                            .unwrap_or("Choisir"),
                    )
                    .show_ui(ui, |ui| {
                        for (idx, v) in app.voitures.iter().enumerate() {
                            ui.selectable_value(
                                &mut app.rep_voiture_idx,
                                idx,
        ...
        if ui.button("Enregistrer l'opération mécanique").clicked() {
            ...
            app.rep_description.clear();
            app.rep_prix.clear();
            app.rep_observation.clear();
    ...
    ui.add_space(15.0);
    ui.label(
        RichText::new("📋 Historique des Interventions Mécaniques")
            .size(16.0)
            .strong(),
    );
    ui.add_space(5.0);
                    ...
                    ui.horizontal(|ui| {
                        ui.vertical(|ui| {
                            ui.label(
                                RichText::new(format!(
                                    "🔧 Voiture: {} ({})",
                                    rep_item.voiture_modele, rep_item.voiture_plaque
                                ))
                                .bold(),
                            );
                            ui.label(format!(
                                "Date: {} | Intervention: {}",
                                rep_item.date, rep_item.description
                            ));
                            if !rep_item.prix.is_empty() {
                                ui.label(
                                    RichText::new(format!("Coût: {} DA", rep_item.prix))
                                        .color(AMBER),
                                );
                            }
    ...
    if let Some(id_to_del) = app.rep_suppr_confirm {
        app.reparations.retain(|x| x.id != id_to_del);
        app.rep_suppr_confirm = None;
        sauvegarder(&reparations_file(), &app.reparations);
    }
}
