...
pub fn page_tableau(ui: &mut egui::Ui, app: &mut App) {
    ...
    ui.horizontal(|ui| {
        panneau().show(ui, |ui| {
            ui.vertical(|ui| {
                ...
                ui.label(
                    RichText::new(format!("{:.0} DA", app.ca_total()))
                        .size(22.0)
                        .bold()
                        .color(GREEN),
                );
        ...
        panneau().show(ui, |ui| {
            ui.vertical(|ui| {
                ui.label(
                    RichText::new("Chiffre d'Affaires du Mois")
                        .size(12.0)
                        .color(muted()),
                );
                ui.label(
                    RichText::new(format!("{:.0} DA", app.ca_mois()))
                        .size(22.0)
                        .bold()
                        .color(GREEN),
                );
        ...
        panneau().show(ui, |ui| {
            ui.vertical(|ui| {
                ui.label(
                    RichText::new("Véhicules Loués Aujourd'hui")
                        .size(12.0)
                        .color(muted()),
                );
                ui.label(
                    RichText::new(format!("{}", app.loues_auj()))
                        .size(22.0)
                        .bold()
                        .color(RED_IAM),
                );
    ...
    ui.add_space(20.0);
    ui.label(
        RichText::new("Alertes Mouvements du Lendemain (Départs / Retours)")
            .size(16.0)
            .strong()
            .color(text()),
    );
    ...
    if retours.is_empty() && departs.is_empty() {
        ui.label(
            RichText::new("Aucun mouvement prévu pour demain.")
                .italic()
                .color(muted()),
        );
    } else {
        if !retours.is_empty() {
            ui.label(
                RichText::new(" Retours Attendus Demain :")
                    .color(AMBER)
                    .strong(),
            );
            for c in &retours {
        ...
        if !departs.is_empty() {
            ui.label(
                RichText::new(" Départs Prévus Demain :")
                    .color(GREEN)
                    .strong(),
            );
            for c in &departs {
