...
pub fn page_voitures(ui: &mut egui::Ui, app: &mut App) {
    ...
    panneau().show(ui, |ui| {
        ...
        egui::Grid::new("v_add")
            ...
            .show(ui, |ui| {
                ...
                egui::ComboBox::from_id_source("v_cat_cb")
                    .selected_text(&app.v_cat)
                    .show_ui(ui, |ui| {
    ...
    let pool = &app.voitures
        [start.min(app.voitures.len())..(start + ITEMS_PER_PAGE).min(app.voitures.len())];
                ...
                panneau().show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ui.vertical(|ui| {
                            ...
                            ui.label(format!(
                                "Plaque: {} | Catégorie: {} | Couleur: {}",
                                v.plaque, v.categorie, v.couleur
                            ));
                            ui.label(
                                RichText::new(format!("Tarif: {:.0} DA/Jour", v.tarif_jour))
                                    .color(GREEN),
                            );
                            ...
                            if let Some(c) = active_c {
                                ui.label(
                                    RichText::new(format!(
                                        "Status: Loué (Contrat: {} - Client: {})",
                                        c.numero, c.client_nom
                                    ))
                                    .color(RED_IAM),
                                );
                            } else {
                                let et_cl = if v.etat == "En maintenance" {
                                    AMBER
                                } else {
                                    GREEN
                                };
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ...
                            if v.etat == "Bon état" {
                                if bouton_neutre(ui, "Maintenance", 100.0) {
                                    if let Some(found) =
                                        app.voitures.iter_mut().find(|x| x.id == vid)
                                    {
    ...
    if let Some(to_rem) = app.v_suppr_confirm {
        app.voitures.retain(|x| x.id != to_rem);
        app.v_suppr_confirm = None;
        sauvegarder(&cars_file(), &app.voitures);
    }

    pagination_controls(ui, &mut app.v_page, app.voitures.len());
}
